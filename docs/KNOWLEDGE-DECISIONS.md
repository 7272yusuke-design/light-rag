...
## 2026-07-30: pixelrag-startrail-l3 — L3投入

- **対象:** https://github.com/StarTrail-org/PixelRAG
- **判断:** L3投入(pixelrag-startrail-l3)
- **根拠:** 文書をパースせずスクリーンショットとしてレンダリングし、VLM(Qwen3-VL-Embedding-2B)で直接埋め込んでFAISS索引化する「ピクセルネイティブ検索」フレームワーク。Apache-2.0。投入価値は4点: (1)図面・複雑表・スキャンPDF等「テキスト抽出で構造が壊れる文書」に対する代替経路として、既存のLightRAG(テキスト+グラフ)を補完する部品になる。(2)MCPサーバもバックエンドも立てずBash経由でrenderを呼びタイル画像を読ませるClaude Codeプラグインの実装例。MCP常駐・認証・トンネル運用コストをゼロにしてエージェントに視覚を与える最小構成の参照。(3)render/embed/index/serveが各段で単体完結する分離設計は、index-first retrievalと同系の設計思想の外部確認事例。(4)LoRAアダプタと学習データ全量をHFで公開しており、埋め込みモデルのドメイン適応の参照実装になる。重複チェック(naive 2クエリ: リポジトリ名 / スクリーンショット・VLM・視覚検索系ドメイン語)で既存エントリなしを確認済み。
- **注記:** 制約: embed/indexは実質GPU必須(device: cuda前提)。Hostinger KVM2(2vCPU/8GB/GPUなし)では索引構築が実行不可のため、自環境で採用可能なのはpixelrag-render単体とClaude Codeプラグインのパターンのみ。serveはCPU可だが事前構築済み索引が前提でWikipedia索引はS3同期のため大容量。trainはルートワークスペースから入れると壊れる(独自のCUDAピン留め環境を持つためtrain/配下から入れる)。成熟度は早期段階(star 28 / fork 0 / リリースなし / コントリビュータ3名)、node_modulesがコミットされており言語構成JSON 93%は実コード量の小ささを意味する。ただし動作するフルパイプライン・HF公開モデル/データセット・828万記事規模のベンチマークという実体があり、コンセプト実装ではないと判断してL3投入。LightRAGの置換ではなくパース不能文書に対する第二経路として位置づける。</notes>
<parameter name="related">layout-preserving-retrieval-hybrid-l2c_lightrag.txt(本L3を経路Bの実装候補とする二経路パターン) / index-first retrieval パターン(段階分離設計の外部確認事例)
- **再検討条件:**
  1. GPU搭載環境(クラウドGPUインスタンス含む)を調達し、レイアウト依存文書の索引構築が実行可能になった時点
  2. 顧客案件で図面・平面図・複雑帳票・スキャンPDFの検索要件が発生し、テキスト抽出型RAGで精度が出ないと判明した時点
  3. 物件マイソク/平面図系の案件で、完成イメージ生成ではなく物件資料の横断検索が要件に入った時点
  4. Claude CodeにMCPを介さず視覚を与える構成を実案件で採用検討する時点
  5. リポジトリが最初のリリースタグを切るか、コントリビュータ・fork数が実質的に増加して破壊的変更リスクが下がった時点
  6. 逆トリガー: 6ヶ月以上コミットが停止し開発放棄と判断できる時点(L3から除去を検討)

---

## 2026-07-30: layout-preserving-retrieval-hybrid-l2c — L2c投入

- **対象:** https://github.com/StarTrail-org/PixelRAG
- **判断:** L2c投入(layout-preserving-retrieval-hybrid-l2c)
- **根拠:** PixelRAG読解から立ち上がった候補パターン。テキスト抽出前提のRAGが構造を失う文書群(図面・平面図・複雑表・スキャンPDF・レイアウト自体が意味を持つ資料)に対し、単一経路への統合ではなく二経路併置で解く構成。経路A=既存LightRAG(テキスト+グラフ+pgvector、無変更)、経路B=レンダリング→タイル化→VLM埋め込み→FAISS。振り分けは投入時に一度確定し、両経路のドキュメントに同一のcustomer ID / document IDを埋めて紐付け、上位の統合クエリ層(既存構成ではHermes相当)でスコアをマージする。中核の設計判断はpgvectorとFAISSを統合しないこと(埋め込み次元・モデル・距離計算が異なるため混ぜると双方の検索品質が劣化する)、および経路Bは追加であって置換ではないこと。既存の「LightRAG全ドキュメントにcustomer IDを埋める」方針の横展開として成立する。</reason>
<parameter name="notes">status:unverified。GPU環境が前提条件のため自環境実証に至っていない。投入前に潰すべき未検証論点5件を本文に明記: (1)テキスト埋め込みのコサイン類似度とVLM埋め込みのFAISS距離のスコア正規化方法、(2)「パースで壊れるか」の自動振り分け判定の精度、(3)両経路への重複投入時の結果重複抑制、(4)GPU調達の損益分岐件数、(5)二経路マージの応答速度がP95要件に収まるか。GPUなしでも進められる段階1-2(レンダリング単体の情報保持性の目視確認 / 振り分け判定ルール策定と対象件数の実数把握)を先に回す設計にしてあり、段階2で「そもそも二経路が必要な規模か」の判断が付く。見送りになる可能性も織り込み済み。L2昇格条件は段階3以降の自環境実証によるverified:self付与、スコア正規化方法の確定、1人運用での保守コスト持続性の実証。
- **関連:** pixelrag-startrail-l3_lightrag.txt(経路Bの実装候補) / index-first retrieval パターン(経路Bも索引で当てて詳細タイルを取る構造を持つ)
- **再検討条件:**
  1. GPU環境を調達し経路Bの小規模索引構築が実行可能になった時点(段階3着手)
  2. 段階1-2(レンダリング単体検証・振り分け判定ルール策定)を実施し、経路Bに回すべき文書件数の実数が判明した時点
...
## 2026-08-01: layout-preserving-retrieval-hybrid-l2c v1.1(対象拡張) — L2c投入

- **対象:** https://github.com/StarTrail-org/PixelRAG
- **判断:** L2c投入(layout-preserving-retrieval-hybrid-l2c v1.1(対象拡張))
- **根拠:** v1.0(静的文書のみ)を時間軸メディア(画面録画動画)へ対象拡張。新規L2cを立てず既存エントリの拡張とした理由は、構造が完全に同一(振り分け → 二経路 → 共通キー紐付け → 統合クエリ層)であり、動画は新しいパターンではなく新しい適用対象にすぎないため。新規作成すると同一原理が3エントリに分散し、既にバックログにある同テーマ重複調査を悪化させる。中核命題「テキスト化の過程で失われる情報は、テキスト化せずピクセルのまま索引する」をセクションとして新設し、静的文書と時間軸メディアを一つの原理の下に統合した。動画版の結合キーは timestamp(静的文書版の customer ID / document ID を引き継ぐ)。画面録画が最適適用対象である根拠2点: (1)PixelRAG公開LoRAアダプタ(wiki-screenshot-embedding-lora)はWebページのスクリーンショットで学習されており、チュートリアル・デモ・ウェビナー・会議の画面共有のフレームは分布上ほぼ同一。追加学習なしで実用精度が出る可能性が高い。逆に実写映像は分布外でありベースモデルまたは別バックボーンへの切り替え判断が必要。(2)画面録画は静止時間が長くdedup圧縮率が極端に高いため、GPU損益分岐が静的文書版より大幅に有利。結果として本パターンは静的文書単体より画面録画を含めた方が成立しやすいという逆説的性質を持つ。既存資産との相互補完が成立することも記録: claude-video のフレーム重複排除(16x16グレースケール + 平均絶対差分)がPixelRAG側のフレーム数爆発を抑え、PixelRAGの索引が既存動画パイプラインの「フレームを抽出時に一度見て捨てる」構造(視覚索引が永続化されず後から画面を引けない)を埋める。片方だけでは成立しない組み合わせである点が記録価値の中心。PixelRAGの5パッケージ分離設計により render のみを動画フレーム抽出に置換し embed/index/serve を無変更で再利用できる。
- **注記:** status:unverified 継続。時間軸メディア固有の未検証論点を4件追加(通算10件): (7)dedupは代表フレームのみ残すため各フレームがカバーする時間区間を点ではなく区間で保持しないと文字起こしとの対応がずれ結合キーが機能しない、(8)文字起こしが既に言及している内容を画面も映すケースでの経路間冗長の抑制ルール、(9)視覚検索が正しい瞬間を当てたかの正解データがなくRagas系指標をそのまま適用できない、(10)混在コーパスでの画面録画/実写の自動判別手段。GPU不要の検証段階として段階1v(画面録画1本のdedup圧縮率実測 → GPU損益分岐の分母確定)、段階2v(dedup後フレームをVLMに直接読ませ文字起こしに現れない情報を何件拾えるか計数 → 便益の定量化)、段階2v'(時間区間の持ち方の机上検証)を追加。段階1v-2vは静的文書系の段階1-2より先に着手する価値があると明記(圧縮率が高く損益分岐が有利、便益も件数として数えやすい)。適用しない条件に実写映像主体・静止区間が少ない動画を追加。

【更新手順の記録】upload_document の overwrite:true は HTTP 404(Error deleting old doc)で失敗。ECC L3 v2.0.0 に続き2件目の確認となり、更新経路が構造的に壊れていることが確定した。ただし今回の検証で重要な挙動が判明: overwriteは削除段で失敗した時点で中断し挿入まで到達しないため、重複レコードは生成されない(失敗は安全側)。psql直接操作で置換を実施。手順: pg_dump(369MB) → doc_name で対象特定(doc-b0a2b7e29023531924aabd81ef96d801) → doc_status.chunks_list から派生チャンクID取得 → トランザクション内で doc_full / doc_status / doc_chunks / vdb_chunks を削除、entity_chunks / relation_chunks は行削除ではなく jsonb 配列から該当チャンクIDのみ除去し空配列になった行のみ削除 → 検証SELECT → COMMIT。結果: 影響42エンティティ中31件・52リレーション中51件が当該文書専属で削除、残りは他文書と共有のため保持。full_entities / full_relations / vdb_entity / vdb_relation は他文書共有ノードのため非対象とし、孤立ノードは再投入時の同名マージに委ねた。

【運用上の注意2件】(1)docker ps -qf name=postgres は lightrag-postgres と geo-postgres の2件にマッチしコマンドが壊れる。コンテナ名を直書きすること。(2)docker exec に -it を付けるとHostingerウェブターミナルでpsqlの出力が表示されない。-it を外すこと。

【別途調査事項】lightrag_doc_full が246件に対し把握件数は221件で25件差がある。workspace別・doc_name重複でのカウントが必要。
- **関連:** pixelrag-startrail-l3_lightrag.txt(経路Bの実装候補) / claude-video-l3_lightrag.txt(フレーム重複排除・トークン予算設計) / ffmpeg-l3_lightrag.txt(フレーム抽出・トラック分離) / meetily-l3_lightrag.txt(会議録音の文字起こし、画面共有検索の適用先) / video-to-knowledge-pipeline-l2c_lightrag.txt(動画→構造化MD→投入。本パターンはその下流に視覚索引を追加する関係) / index-first retrieval パターン
- **再検討条件:**
  1. 段階1v(画面録画のdedup圧縮率実測)を実施し、1時間あたりの代表フレーム数が判明した時点
  2. 段階2v(文字起こしに現れない情報の計数)で便益が定量化され、GPU調達の損益分岐が計算可能になった時点
  3. meetily で蓄積した会議録画に対し、画面共有内容の検索要件が実際に発生した時点
  4. 顧客案件で操作説明動画・オンボーディング動画の横断検索が要件化した時点
  5. GPU環境を調達し経路Bの小規模索引構築が実行可能になった時点(段階3着手)
  6. Hermesの統合クエリ層が複数索引のマージに対応する改修を行う時点
  7. 逆トリガー: 段階1v-2vの実測でdedup圧縮率が想定より低い、または文字起こしに現れない情報がほとんど拾えないと判明した時点(動画拡張部分を見送りへ変更しv1.0相当に戻す)

---

## 2026-08-01: first-principles-of-agent-development-l0(遡及記録 + v2.0分割再構成の決定) — 保留

- **対象:** internal:LightRAG KB横断合成(L2/L3群18件、外部リポジトリなし)
- **判断:** 保留(first-principles-of-agent-development-l0(遡及記録 + v2.0分割再構成の決定))
- **根拠:** 【遡及記録】2026-07-09にLightRAG L2/L3群18件からの横断合成(記憶・マルチエージェント・コンテキスト工学・評価/自己改善・過剰設計抑制の5テーマ)で作成・投入されたL0エントリ。投入時にKNOWLEDGE-DECISIONS.mdへの記録・識別子採番・4項目品質基準チェックが実施されておらず、L0運用ルール8.1の外側で投入されていた。2026-08-01の台帳監査(doc_full 247件 vs 台帳222件の差分調査)で発見。

【評価結果】内容はL0適格でありKB内で最もL0らしい文書と判定。「3つの不変制約(コンテキスト有限/出力確率的/複雑さ複利)→7原則→メタ原則(原則と流儀の判別法・収斂進化テスト)」という導出構造はL0定義の理想形に近く、Phase 1の6件より上位の抽象度を持つ。メタ原則はpattern-type:meta-cognitionの定義そのもの。ただし文書としては品質基準違反が3件: (1)抽象度違反(基準6.2)— DSPy/Inspect AI/Langfuse/Letta/Stagehand/wshobson等のフレームワーク名が証拠として全編に織り込まれ、combination-architect/skill-verifier/loop-architect等のYusuke環境固有スキル名も本文に混入。(2)適用範囲の明示欠落(基準6.3)— 適用条件/適用しない条件の両セクションが不在で7原則が万能視されるリスク。(3)ガバナンス違反(運用ルール8.1)— DECISIONS記録なし・識別子未採番。

【階層矛盾の発見】本文書の原則1はProgressive Disclosureを「コンテキスト経済という原則の一実装」と位置づけるが、Phase 1計画のL0-007はPDを独立したL0原理として投入予定。このまま6件を投入するとL0レイヤー内部で「PDは原理」「PDは原理の実装」という矛盾した階層観が併存し、思考の足場を売る層としての商品品質を毀損する。原則3(maker≠checker)とL0将来候補-A(責務分離)、原則6(蒸留)とL0-006(3層メモリ)にも重なりの兆候あり。

【ガバナンス欠陥の発見】L0設計は制約5でVaultからの抽出を基本方針とするが、本文書はKB内部からのボトムアップ合成という設計時未想定の生成経路で生まれた。L1→L2c→L2の昇格ルールは存在するがL2→L0の昇格ゲートが未定義であり、本件はその未定義経路を通った最初の事例。経路自体は本文書の原則6(蒸留・昇格パイプライン)の実演であり健全なため、禁止ではなくゲートの明文化(3つの問い+4項目品質基準+DECISIONS記録を昇格条件とする)で対処する。
- **注記:** 【決定(Yusuke判断 2026-08-01)】削除・降格・現状維持を退け、Phase 1の6件投入と同バッチでのv2.0分割再構成を採用。理由: 6件は相互参照整理を同時実施する計画のため、本文書のv2.0化と相互参照を一度で済ませられる。

【v2.0分割計画】3層に分解する。(1)L0本体に残すもの: 3制約・7原則・メタ原則・アンチパターン。ツール名を除去し、適用条件/適用しない条件を追加、ファイル仕様5.2の構造に整形、識別子を採番。(2)ツール名による証拠群: 「関連レイヤーとの接続」セクションでL3エントリへの参照リンクに降格(証拠は消さず場所を変える)。(3)運用への組み込み(自スキル連携): 環境固有の運用ルールとしてL1へ分離。併せてL0-007との関係を「Progressive Disclosureはコンテキスト経済の名前付き具体原理」として明文接続し階層矛盾を解消する。原則3とL0将来候補-A、原則6とL0-006の重なりも同バッチで整理する。

【残作業】v2.0化まで現行版はL0としてDB内に残置(検索には引き続きヒットする)。L2→L0昇格ゲートの明文化はL0-LAYER-DEFINITION.md v1.2改訂として6件投入時に反映する。
- **関連:** L0-LAYER-DEFINITION.md v1.1 / KNOWLEDGE-DECISIONS-L0-section-draft-v1.3(Phase 1の6件) / completeness-as-spectrum-l0(L0-008ドラフト) / agreement-validity-assessment-l0(L0-010ドラフト) / 合成元18件(multi-agent-design-patterns-l2, superpowers-full-l2, agent-memory-design-principles-l2 ほか)
- **再検討条件:**
  1. SKILL改訂(Step 4)が完了しPhase 1の6件投入バッチに着手する時点(v2.0分割を同バッチで実施)
...
## 2026-08-12: shannon-keygraph-l3 — L3投入

- **対象:** https://github.com/KeygraphHQ/shannon
- **判断:** L3投入(shannon-keygraph-l3)
- **根拠:** Claude Agent SDKを実運用規模で動かした数少ない公開実装。セキュリティ診断ツールとしてではなく、長時間自律エージェントの実装リファレンスとしてL3投入。主要参照点は4つ: (1) workspace + gitコミット粒度のチェックポイント/レジューム設計 — L0-006(3層メモリ)の実行時記憶の具体実装、(2) Temporal + 使い捨てDockerワーカー(docker run --rm)+ 呼び出し単位タスクキューによる実行隔離、(3) Phase3(仮説生成)とPhase4(実証)を別エージェントに分離したmaker≠checker構造、"No Exploit, No Report"という検証可能な停止条件、(4) small(Haiku)/medium(Sonnet)/large(Opus)の3ティアモデル割り当てと約$50/回・1〜1.5hという実測コストレンジ。加えてlogin_flowの自然言語ステップ+プレースホルダ置換は非エンジニア向け入力設計の参照、Shannon Proのデータプレーン(顧客インフラ)/コントロールプレーン(ベンダ)分離は企業向けAIサービスの分離設計の参照。重複チェック3ステップ実施済み(naive: リポジトリ名/URL、hybrid: セキュリティ診断・マルチエージェント・Temporal)、既存エントリなし・セキュリティ診断ドメイン自体が未カバーであることを確認。規模はstar 44k/fork 5.1k、v1.4.0(2026-05-28)、TypeScript 92.5%、XBOWベンチマーク96.15%(100/104)。制約として本番実行禁止(破壊的副作用あり)、書面許可必須、ホワイトボックス専用、prompt injection耐性なし、AGPL-3.0(SaaS提供時は改変公開義務)、カバー範囲はAuth/Authz・Injection・XSS・SSRFのみ、WindowsはWSL2のみをナレッジ本体に明記。
- **注記:** purpose:reference-primary。実運用ツールとしてではなく実装リファレンスとしての投入。L2c候補メモをナレッジ本体末尾に記載(チェックポイント/レジューム設計の切り出しは別実装との突合後に判断)。Router Mode(claude-code-router)はsunset予定でLiteLLM等のAnthropic互換プロキシへの移行が必要な点も記載済み。
- **関連:** L0-006(3層メモリアーキテクチャ) / L0-009(自律実行の責任境界) / loop-architect SKILL / openmontage-calesthio-l3 / ecc-l3
- **再検討条件:**
  1. クライアント案件でWebアプリのセキュリティ診断・脆弱性検証が要件として発生した時点(Lite実運用の可否をAGPLとコスト$50/回で再評価)
  2. 自社開発物(OpenClaw等)を外部公開する前段でステージング環境の診断が必要になった時点
...
## 2026-08-23: openviking-l3 — L3投入

- **対象:** https://github.com/volcengine/OpenViking
- **判断:** L3投入(openviking-l3)
- **根拠:** star 26.9k / commit 1,753 / releases 59 / v0.4.10(2026-07-16)、Volcengine(ByteDance)製のコンテキストデータベース。VLDB 2026採択論文(VikingMem、arXiv:2605.29640)の裏付けを持つ。四つの理由でL3投入。(1)LightRAGとの直接比較データを持つ唯一のエントリ: READMEにLightRAGを名指しした比較表があり、自社基盤の相対的な立ち位置を数値で把握できる。特に L0-PRODUCT-CONSTRAINTS.md 制約4 が定めるSLA(応答速度P95 2秒以内)に対し、LightRAGの検索遅延9.19秒(5データセット平均)・75秒(HotpotQA)という数値は、商品化フェーズで応答速度が構造的課題になる可能性を示す。自社計測ではないため条件差の考慮は必要だが、直視すべき数値として記録した。(2)自社スタックへの直撃データ: LoCoMoベンチマークで OpenClaw + OpenViking が24.20%→82.08%(精度3.39倍、トークン-91.0%)、Hermes + OpenViking が33.38%→82.86%(精度2.48倍)、Claude Code + OpenViking が57.21%→80.32%(トークン-63.2%、遅延-58.45%)。OpenClaw・Hermes・Claude Codeはいずれも自社で使用中であり、特にClaude Codeのトークン63.2%削減は日常的な開発コストに直接効く。(3)LightRAG運用の弱点への構造的回答: 検索軌跡の完全保存・可視化により「なぜこれがヒットしたか分からない」問題が解消される設計。ディレクトリ再帰検索(意図解析→ベクトルで高スコアディレクトリを特定→ディレクトリ内で二次検索→サブディレクトリへ再帰→集約)は、フラットなチャンク検索と異なり情報が置かれた文脈全体を理解する。(4)設計思想の参照価値: ファイルシステムパラダイム(viking:// 配下に記憶・リソース・スキルを統合、ls/find/grep/treeで決定論的に操作)は「曖昧な意味マッチを追跡可能なファイル操作に変える」発想。ディレクトリごとに .abstract / .overview が入れ子で存在する構造は、L0-007(Progressive Disclosure)の再帰的実装例として book-to-skill(1段階)より進んだ形。セッション終了時の非同期記憶抽出による自己反復は L0-006(3層メモリアーキテクチャ)の実装例。Ollama対応で完全ローカル運用が可能、OpenViking Helper が Claude Code / Codex / Cursor / Trae / OpenCode を検出して plugin・MCP・Hook 統合を設定できる点も実務的。
- **注記:** purpose:reference-primary + purpose:competitive-analysis。⚠️用語衝突対策を最優先で実施: OpenVikingも「L0/L1/L2の3層」を使うが本ナレッジベースのレイヤー定義と全くの別物(OpenViking側は L0=Abstract 1文要約約100トークン / L1=Overview 構造と要点約2kトークン / L2=Details 全文オンデマンド)。ナレッジ本体の冒頭に「読む前に必ず確認」として対比表を配置し、本文中では一貫して OV-L0 / OV-L1 / OV-L2 と表記して自社レイヤーと区別した。acp-agent-commerce-protocol-l2c の名称衝突と異なり、今回は自社の中核概念そのものが衝突するため、検索時の混線リスクが格段に高いと判断した措置。LightRAGとのベンチマーク比較を単純な優劣として記載せず「用途分離の判断材料」として位置付けた点も重要: 5データセット平均ではLightRAGが精度で勝つ(76.00% vs 66.87%)一方、インデックス構築コストは7.2倍・検索遅延は48倍。マルチホップ(HotpotQA)ではOpenViking top-20が逆転(91.00% vs 89.00%)。「LightRAGは重いが賢い、OpenVikingは軽くて速いが平均精度は劣る」という整理をナレッジ本体に明記した。またベンチマークが提供元の自社計測である点、5データセットの選定に偏りがある可能性が未検証である点をリスクとして併記した。重複チェック3ステップ実施済み、既存エントリなし。L2c候補メモ「知識の階層化を人が判断するか自動生成するか」を記録、自社レイヤー設計思想の根幹に関わる論点だがPoC未実施のため保留。
- **関連:** LightRAG自社基盤(直接比較対象) / ragflow-infiniflow-l3 / milvus-l3 / book-to-skill-l3 / hermes-agent-l3 / OpenClaw(自社プロジェクト) / L0-006(3層メモリアーキテクチャ) / L0-007(Progressive Disclosure) / L0-PRODUCT-CONSTRAINTS.md制約4
- **再検討条件:**
  1. VLM推論のリソース要件を実測する時点(GPUなしKVM2でVLMが現実的に動くかが導入可否の最初の関門。ここが通らなければ以降の検討は不要)
  2. Claude Codeのトークン消費が課題として顕在化した時点(-63.2%削減の実測値を自環境で検証)
  3. LightRAGの検索遅延がSLA(P95 2秒以内)の制約として顕在化した時点(用途分離の判断材料として本エントリの比較表を使用)
  4. OpenClawプロジェクトでエージェントメモリの設計に着手する時点(精度3.39倍・トークン91%削減の計測値を前提に検討)
  5. ナレッジMCPサービスの商品設計で「検索の説明可能性」が要件化した時点(検索軌跡の可視化を参照)
...
## 2026-09-17: presenton-l3 — L3投入

- **対象:** https://github.com/presenton/presenton
- **判断:** L3投入(presenton-l3)
- **根拠:** 顧客提供向けの提案書・定型レポート自動生成基盤として投入。Apache 2.0で制限なし、Docker/APIサービスとして常駐でき、n8nのHTTPノードから無人実行できる点が主軸事業（n8n自動化）と直結する。既存PPTXをアップロードしてテンプレート化できるためブランド統一が可能。Ollama/LM Studio対応で資料内容を外部に出せない顧客にも提供可能。既存の ppt-master（対話型SKILL・一点物の高品質資料向け）とは性質が異なり、Presentonは定型の反復生成に強いという棲み分けをエントリ内に明記。
- **注記:** purpose:product。制約として日本語体裁の未検証、テンプレート整備工数、匿名テレメトリの既定有効（顧客納品時は必ず無効化）、画像生成の従量コストを記載。参考価格帯（外部動画の想定値・米国基準）初期$2,500/月額$200。
- **関連:** ppt-master-l3, officecli-l3, ai-document-generation-pipeline-l2
- **再検討条件:**
  1. 日本語スライド（フォント埋め込み・禁則処理・テンプレート体裁）の実測で顧客納品水準に達しないと判明した時点
  2. 実案件でPresentonとppt-masterを併用し、使い分け基準がL2候補として抽出できた時点
  3. Apache 2.0以外へのライセンス変更が発表された時点

---

## 2026-09-17: dify-l3 — L3投入

- **対象:** https://github.com/langgenius/dify
- **判断:** L3投入(dify-l3)
- **根拠:** 企業の社内文書（SOP・ハンドブック・価格表）を取り込んだプライベートチャットボットの顧客納品基盤として投入。既存の Flowise と同カテゴリだが、ライセンス条件が提供形態を分ける決定要因になるため、その判別軸を記録に残すこと自体が投入目的。Difyは Apache 2.0 そのものではなく追加条件付き（Dify Open Source License）で、公式LICENSEは「1テナント=1ワークスペース」と定義しており、1インスタンス上に顧客ごとのワークスペースを作る構成だけでマルチテナント環境に該当し商用ライセンスが必要になる。LOGO改変禁止はフロントエンド（web/ディレクトリ、webイメージ）利用時のみ適用され、API公開モードのみの構成ならホワイトラベル化が可能。この2点をエントリの中核として明記した。
- **注記:** purpose:product。安全な提供形態は「顧客1社につき1インスタンスを顧客環境に分離構築」。SaaS化・ワークスペース分割による複数顧客収容は商用ライセンス必須。意匠特許による保護の記載あり。参考価格帯（外部動画の想定値・米国基準）初期$1,500/月額$250＋Dify Cloud $59/月。
- **関連:** flowise-l3, astrbot-l3, agno-l3, ragas-l3
- **再検討条件:**
...
## 2026-09-17: treg-people-data-l3 — L3投入

- **対象:** https://treg.to
- **判断:** L3投入(treg-people-data-l3)
- **根拠:** 人物・企業データ層はKB259件中に1件も存在しない空白領域だったため投入。treg.toは84プラットフォーム・約2,875エンドポイント（People Data Labs、ContactOut、Crustdata、Lusha、GetLeads.io等）を単一MCPサーバーに束ね、各社の公表レートそのまま（マークアップ$0.000）で前払い残高から従量課金するプロキシ。自社鍵を登録した分は非課金。設計として、ジョブ単位の抽象＋自社鍵優先・最安ルーティング、search（低単価）とreveal（高単価）を分離した二段階探索、live-verified表示と実測成功率の公開が特徴的で、リード発掘パイプライン全般に転用可能な構造を持つ。
- **注記:** purpose:product。オープンソースではなくクローズドな商用SaaS（セルフホスト経路なし、単一事業者依存）。最重要の制約2点：(1) 上流プロバイダがLinkedIn・北米B2B SaaS中心のため日本の中小企業カバレッジが薄いと想定され、日本国内リード発掘の主軸には使えない可能性が高い（日本市場はGoogleマップ起点の方が実効性が高い）。有効なのは越境・外資系SaaS・Shopify利用EC・LinkedIn利用層に限定。(2) 特定電子メール法のオプトイン原則、個人情報保護法の確認記録義務、GDPR/CCPA、各プロバイダの再配布制限。顧客にリードリストそのものを納品する設計はリスクが高く、顧客自身のアカウントで取得させ仕組みの構築・運用を提供する形が安全。
- **関連:** openseo-l3, composio-l3, smithery-l3, openai-compatible-api-abstraction-l2c
- **再検討条件:**
  1. 日本国内の中小企業を対象に実クエリでヒット件数を実測し、カバレッジが顧客提案水準に達するか判明した時点
  2. 顧客にリードリストを納品する形の案件が発生し、個人情報保護法の確認記録義務・各プロバイダの再配布制限について法務確認が必要になった時点
  3. treg.toの価格改定・仕様変更・サービス終了が発表された時点
  4. ジョブ単位ルーティング＋二段階探索の構造をL2c（プロバイダルーティングパターン）として切り出す判断を行う時点

---

## 2026-09-17: multi-provider-data-routing-l2c — L2c投入

- **対象:** https://treg.to
- **判断:** L2c投入(multi-provider-data-routing-l2c)
- **根拠:** treg-people-data-l3 の設計構造から汎用化して抽出した候補パターン。4要素で構成される：(1) エンドポイントではなくジョブ単位の抽象、(2) 自社鍵優先→最安の順でルーティングし応答元を必ず明示、(3) search（低単価・母集団全体）と reveal（高単価・絞り込み後のみ）の二段階分離によるコスト制御、(4) 最終検証日と実測成功率（4xxを除外）による信頼性指標の併記。実装の選択肢として構成A（既製プロキシ利用）/B（n8n自前実装）/C（混成）を提示。単一プロバイダ依存・エンドポイント抽象の判断不能性・探索コストの線形膨張という3問題を解決する。
- **注記:** ファイル名は multi-provider-data-routing-l2c_lightrag.txt。inventory.sh の層分類がファイル名パターンマッチング依存のため、L2c判定が想定どおりに効いているかを次回 inventory.sh 実行時に確認する。既存需要への紐付け：営業リスト自動生成パイプライン設計書（リード収集）を第一の適用先として明記済み。投機的バリデーションではない。
- **関連:** treg-people-data-l3, openseo-l3, openai-compatible-api-abstraction-l2c, n8n-workflow-design-l2
- **再検討条件:**
  1. 実案件で構成A〜Cのいずれかを稼働させ、二段階分離によるコスト削減率が実測できた時点（L2昇格判断）
...
## 2026-09-25: jev-social-l3 — 見送り

- **対象:** https://github.com/socai-io/jev-social
- **判断:** 見送り(jev-social-l3)
- **根拠:** 成熟度不足（star 7 / fork 1 / commit 10 / release なし）。実体はsocaiのデモアプリで、部品在庫としての独立価値が低い。Jev（OpenRouter経由モデル）+ ローカルChrome前提でKVM2同居不可。
- **注記:** Yusuke判断による見送り（2026-09-25）
- **関連:** socai-l3（同日見送り）、L0-009（自律実行の責任境界：型付き操作選択・低確信度不実行・部分結果返却の実装例として参考）
- **再検討条件:**
