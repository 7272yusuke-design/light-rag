...
| レイヤー | 識別子 | 内容 | 投入条件 |
|---------|--------|------|---------|
| L1 | l1-external | 外部OSSの使い方・設計思想 | repomix + LLM要約で投入 |
| L2 | l2-combination | 複数L1の接続パターン | verified以上で投入 |
| L3 | l3-package | 検証済み自作モジュール | production以上で投入 |
...
|------|------|---------|
| concept | アイデア段階 | 不可 |
| experimental | 開発中・未検証 | 不可 |
| verified | 動作検証済み | L2として投入可 |
| production | 本番稼働中 | L3として投入可 |
...
## L3: 独自パッケージテンプレート
    ...
    - 派生元: [L2パターン名 or L1名]
    - カテゴリ: package
    - タグ: original, tested, [技術タグ]
    - バージョン: X.Y
    - リポジトリ: [GitHub URL]
    - 検証状態: production / beta
    - 作成日: YYYY-MM-DD
    - 更新日: YYYY-MM-DD
    ## 概要
    [何をするパッケージか]
